...
const WEB3FORMS_SUBJECT = "New enquiry from AppMaker Egypt website";
...
export function Contact() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      form.reset();
      toast.success(t.contact.sent || "Thanks! We'll reach out shortly.");
    } catch (error) {
      toast.error("Message was not sent. Please try again or contact us directly by email.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mt-14 grid grid-cols-1 gap-8 lg:grid-cols-5">
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Input required name="name" placeholder={t.contact.name} disabled={loading} />
              <Input required type="email" name="email" placeholder={t.contact.email} disabled={loading} />
            </div>
            <Input className="mt-4" name="phone" placeholder={t.contact.phone} disabled={loading} />
            <Textarea required rows={5} className="mt-4" name="message" placeholder={t.contact.message} disabled={loading} />
